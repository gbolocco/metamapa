...
public class AdminController {
  ...
  public AdminController(ServicioFuentes servicioFuentes, ServicioUsuarios servicioUsuarios,
      ServicioColecciones servicioColecciones, ServicioSolicitudes servicioSolicitudes) {
        ...
        if (tipo != null && campoStr != null && valor != null && !valor.isEmpty()) {
          ...
          switch (tipo) {
            ...
            case "fechaDesde":
              try {
                filtros.add(
                    new FiltroFechaDesde(LocalDateTime.parse(valor, formatter), CampoDeHecho.FECHA_ACONTECIMIENTO));
            ...
            case "fechaHasta":
              try {
                filtros.add(
                    new FiltroFechaHasta(LocalDateTime.parse(valor, formatter), CampoDeHecho.FECHA_ACONTECIMIENTO));
              } catch (DateTimeParseException e) {
  ...
  private void mostrarSolicitudesEliminacion(Context ctx) {
    List<Solicitud> solicitudesRepo = servicioSolicitudes
        .obtenerSolicitudesPendientesPorTipo(TipoSolicitud.ELIMINACION_HECHO);
  ...
  private void mostrarTodasLasSolicitudes(Context ctx) {
    ...
    if (todasSolicitudes.isEmpty()) {
      todasSolicitudes.addAll(List.of(
          ...
          Map.of("id", 102, "titulo", "Solicitud de Eliminación - Inundación Errónea", "fecha", "2025-10-29 16:30",
              "solicitante", "Servicio Meteorológico", "tipo", "Eliminación", "estado", "Pendiente")));
    }
  ...
  private void renderizarSolicitudes(Context ctx, List<Map<String, Object>> solicitudes, String titulo,
      String tipoActivo) {
    String pageParam = ctx.queryParam("page");
    int page = pageParam != null ? Integer.parseInt(pageParam) : 1;
    int pageSize = 10;
